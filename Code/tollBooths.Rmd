...
recovery_period<-cars%>%
  select(tollbooth,Single.Journey,Actual.Traffic,Capital.cost,funding.Model)
...
```{r results="asis"}
x<-booth_tidy%>% group_by(funding.Model)%>%summarize(Number.Of.Booth=n())%>%arrange(desc(Number.Of.Booth))
print( xtable(x , caption="Funding model of toll roads"),type="html")
```
```{r results="asis"}
ggplot(cpkm,aes(cost.per.km))+geom_histogram()+facet_wrap(~funding.Model)+ylab("Number of Toll roads")
print(xtable(
joined%>%na.omit()%>%group_by(funding.Model)%>%summarise(Average.Cost.Crore=mean(Capital.cost/crore))
,caption="Average cost"),type="html")
```
...
* After that, toll building started taking off in the 2000's with the maximum number of toll booths commissioned during the UPA-2 government.
...
* The average cost per km of road is `r mean(cpkm$cost.per.km)/crore` Crores. min is `r min(cpkm$cost.per.km)/crore` max is `r max(cpkm$cost.per.km)/crore` median is `r median(cpkm$cost.per.km)/crore`
...
* `r sum(x$Actual.Traffic)/lakh` lakh vehicles use these toll booths daily.
* `r 365*sum(x$Actual.Traffic*x$Single.Journey)/(lakh*crore)` Lakh Crore Is the total amount that should be earned this year based on actual average daily traffic. i.e. On averge cost recovery for the toll road takes place within `r sum(is.na(cost_tidy$Capital.cost))/(365*sum(x$Actual.Traffic*x$Single.Journey))` years.
`r #* At this rate the total investment in all toll roads should be recovered in r (sum(cost_tidy$Capital.cost,na.rm=T) -earning2date)/ (365*sum(x$Actual.Traffic*x$Single.Journey))  years.`
...
```
On an average Public funded projects get completed at a lower cost compared to project under BOT( Build Operate Transfer)
